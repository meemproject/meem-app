...
export const AndroidLinkExtensionSettings: React.FC = () => {
	const { classes: meemTheme } = useMeemTheme()
	const { agreement, isLoadingAgreement } = useAgreement()
	const agreementExtension = extensionFromSlug('android', agreement)
	const sdk = useSDK()
	...
	const saveChanges = async () => {
		if (
			linkUrl.length === 0 ||
			linkUrl.length > 100 ||
			!linkUrl.includes('https://play.google.com')
		) {
	...
	return (
		<div>
			<ExtensionBlankSlate extensionSlug={'android'} />
			{extensionIsReady(
				isLoadingAgreement,
				agreement,
				agreementExtension
				...
				<>
					...
					{agreement?.isCurrentUserAgreementAdmin && (
						<div>
							<ExtensionPageHeader extensionSlug={'android'} />
							...
							<Container>
								<div>
									...
									>
										{`Google Play Store Link URL`.toUpperCase()}
									</Text>
									<Space h={12} />
									<TextInput
										radius="lg"
