...
export const OpenSeaLinkExtensionSettings: React.FC = () => {
	const { classes: meemTheme } = useMeemTheme()
	const { agreement, isLoadingAgreement } = useAgreement()
	const agreementExtension = extensionFromSlug('opensea', agreement)
	const sdk = useSDK()
	...
	const saveChanges = async () => {
		if (
			linkUrl.length === 0 ||
			linkUrl.length > 100 ||
			!linkUrl.includes('https://opensea.io/')
		) {
	...
	return (
		<div>
			<ExtensionBlankSlate extensionSlug={'opensea'} />
			{extensionIsReady(
				isLoadingAgreement,
				agreement,
				agreementExtension
				...
				<>
					...
					{agreement?.isCurrentUserAgreementAdmin && (
						<div>
							<ExtensionPageHeader extensionSlug={'opensea'} />
							...
							<Container>
								<div>
									...
									>
										{`OpenSea Collection URL`.toUpperCase()}
									</Text>
									<Space h={12} />
									<TextInput
										radius="lg"
