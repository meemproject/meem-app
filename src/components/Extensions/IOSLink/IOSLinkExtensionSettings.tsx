...
export const IOSLinkExtensionSettings: React.FC = () => {
	const { classes: meemTheme } = useMeemTheme()
	const { agreement, isLoadingAgreement } = useAgreement()
	const agreementExtension = extensionFromSlug('ios', agreement)
	const sdk = useSDK()
	...
	const saveChanges = async () => {
		if (
			linkUrl.length === 0 ||
			linkUrl.length > 100 ||
			!linkUrl.includes('https://apps.apple.com')
		) {
	...
	return (
		<div>
			<ExtensionBlankSlate extensionSlug={'ios'} />
			{extensionIsReady(
				isLoadingAgreement,
				agreement,
				agreementExtension
				...
				<>
					...
					{agreement?.isCurrentUserAgreementAdmin && (
						<div>
							<ExtensionPageHeader extensionSlug={'ios'} />
							...
							<Container>
								<div>
									...
									>
										{`App Store Listing URL`.toUpperCase()}
									</Text>
									<Space h={12} />
									<TextInput
										radius="lg"
