...
export const InstagramLinkExtensionSettings: React.FC = () => {
	const { classes: meemTheme } = useMeemTheme()
	const { agreement, isLoadingAgreement } = useAgreement()
	const agreementExtension = extensionFromSlug('instagram', agreement)
	const sdk = useSDK()
	...
	const saveChanges = async () => {
		if (
			linkUrl.length === 0 ||
			linkUrl.length > 100 ||
			!linkUrl.includes('https://instagram.com')
		) {
	...
	return (
		<div>
			<ExtensionBlankSlate extensionSlug={'instagram'} />
			{extensionIsReady(
				isLoadingAgreement,
				agreement,
				agreementExtension
				...
				<>
					...
					{agreement?.isCurrentUserAgreementAdmin && (
						<div>
							<ExtensionPageHeader extensionSlug={'instagram'} />
							...
							<Container>
								<div>
									...
									>
										{`Instagram Profile URL`.toUpperCase()}
									</Text>
									<Space h={12} />
									<TextInput
										radius="lg"
