...
export const GitHubLinkExtensionSettings: React.FC = () => {
	const { classes: meemTheme } = useMeemTheme()
	const { agreement, isLoadingAgreement } = useAgreement()
	const agreementExtension = extensionFromSlug('github', agreement)
	const sdk = useSDK()
	...
	const saveChanges = async () => {
		if (
			linkUrl.length === 0 ||
			linkUrl.length > 100 ||
			!linkUrl.includes('https://github.com')
		) {
	...
	return (
		<div>
			<ExtensionBlankSlate extensionSlug={'github'} />
			{extensionIsReady(
				isLoadingAgreement,
				agreement,
				agreementExtension
				...
				<>
					...
					{agreement?.isCurrentUserAgreementAdmin && (
						<div>
							<ExtensionPageHeader extensionSlug={'github'} />
							...
							<Container>
								<div>
									...
									>
										{`GitHub Organization URL`.toUpperCase()}
									</Text>
									<Space h={12} />
									<TextInput
										radius="lg"
