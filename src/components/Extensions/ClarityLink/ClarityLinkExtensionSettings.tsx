...
export const ClarityLinkExtensionSettings: React.FC = () => {
	const { classes: meemTheme } = useMeemTheme()
	const { agreement, isLoadingAgreement } = useAgreement()
	const agreementExtension = extensionFromSlug('clarity', agreement)
	const sdk = useSDK()
	...
	const saveChanges = async () => {
		if (
			linkUrl.length === 0 ||
			linkUrl.length > 100 ||
			!linkUrl.includes('https://app.clarity.so')
		) {
	...
	return (
		<div>
			<ExtensionBlankSlate extensionSlug={'clarity'} />
			{extensionIsReady(
				isLoadingAgreement,
				agreement,
				agreementExtension
				...
				<>
					...
					{agreement?.isCurrentUserAgreementAdmin && (
						<div>
							<ExtensionPageHeader extensionSlug={'clarity'} />
