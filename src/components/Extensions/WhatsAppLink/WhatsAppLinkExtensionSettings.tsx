...
export const WhatsAppLinkExtensionSettings: React.FC = () => {
	const { classes: meemTheme } = useMeemTheme()
	const { agreement, isLoadingAgreement } = useAgreement()
	const agreementExtension = extensionFromSlug('whatsapp', agreement)
	const sdk = useSDK()
	...
	const saveChanges = async () => {
		if (
			linkUrl.length === 0 ||
			linkUrl.length > 100 ||
			!linkUrl.includes('https://chat.whatsapp.com')
		) {
		...
		<div>
			...
			{extensionIsReady(
				...
				<>
					...
					{agreement?.isCurrentUserAgreementAdmin && (
						<div>
							<ExtensionPageHeader extensionSlug={'whatsapp'} />
							...
							<Container>
								<div>
									...
									>
										{`WhatsApp Group URL`.toUpperCase()}
									</Text>
									<Space h={12} />
									<TextInput
										radius="lg"
