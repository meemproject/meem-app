...
export const GDriveLinkExtensionSettings: React.FC = () => {
	const { classes: meemTheme } = useMeemTheme()
	const { agreement, isLoadingAgreement } = useAgreement()
	const agreementExtension = extensionFromSlug('gdrive', agreement)
	const sdk = useSDK()
	...
	const saveChanges = async () => {
		if (
			linkUrl.length === 0 ||
			linkUrl.length > 100 ||
			!linkUrl.includes('https://drive.google.com/')
		) {
		...
		<div>
			...
			{extensionIsReady(
				...
				<>
					...
					{agreement?.isCurrentUserAgreementAdmin && (
						<div>
							<ExtensionPageHeader extensionSlug={'gdrive'} />
							...
							<Container>
								<div>
									...
									>
										{`Google Drive Share URL`.toUpperCase()}
									</Text>
									<Space h={12} />
									<Text className={meemTheme.tExtraSmall}>
										{`Don't forget to make sure the Google Drive sharing permissions are set appropriately!`}
									</Text>
									<Space h={12} />
									<TextInput
										radius="lg"
