...
export const LinkedInLinkExtensionSettings: React.FC = () => {
	const { classes: meemTheme } = useMeemTheme()
	const { agreement, isLoadingAgreement } = useAgreement()
	const agreementExtension = extensionFromSlug('linkedin', agreement)
	const sdk = useSDK()
	...
	const saveChanges = async () => {
		if (
			linkUrl.length === 0 ||
			linkUrl.length > 100 ||
			!linkUrl.includes('https://www.linkedin.com/')
		) {
	...
	return (
		<div>
			<ExtensionBlankSlate extensionSlug={'linkedin'} />
			{extensionIsReady(
				isLoadingAgreement,
				agreement,
				agreementExtension
