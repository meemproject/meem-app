...
export const CAClubLinks: React.FC<IProps> = ({ club }) => {
	const { classes: clubsTheme } = useClubsTheme()

	// Fetch a list of available LINK extensions.
	const {
		loading,
		error,
		data: inteData
	...
	// Lists of extensions
	const [existingLinkExtensions, setExistingLinkExtensions] = useState<
		Extension[]
	>([])
	...
	const updateExtensionLocally = (extraData: any) => {
		...
		if (updatedInte && extensionBeingEdited) {
			...
			if (!extensionBeingEdited.isExistingExtension) {
				...
				const newExtensions = [...existingLinkExtensions]
				...
				setExistingLinkExtensions(newExtensions)
			}
		}
	}
	...
	useEffect(() => {
		...
		if (
			...
			allExtensions.forEach(inte => {
				...
				existingLinkExtensions.forEach(existing => {
	...
	useEffect(() => {
		if (!hasSetupEnabledExtensions) {
			...
			setExistingLinkExtensions(club.allExtensions ?? [])
	...
	return (
		<>
			<div>
				...
				{existingLinkExtensions && existingLinkExtensions.length > 0 && (
					<>
						<Text
							className={clubsTheme.tMediumBold}
						>{`Added apps (${existingLinkExtensions?.length})`}</Text>
						<Space h={12} />
						<Grid>
							{existingLinkExtensions.map(extension => (
								<Grid.Col
									xs={8}
									sm={8}
									md={4}
